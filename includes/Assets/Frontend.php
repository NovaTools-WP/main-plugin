 ...
 * Class Frontend
 *
 * Handles frontend functionalities for the NovaTools.
 *
 * @package NovaTools\Assets
 */
class Frontend {
	 ...
	 */
	const HANDLE = 'novatools-frontend';

	/**
	 * JS Object name for NovaTools.
	 */
	const OBJ_NAME = 'novaToolsFrontend';
	...
	public function enqueue_script( $screen ) {
		...
		if ( in_array( $current_screen, $this->allowed_screens, true ) ) {
			Assets\enqueue_asset(
				NOVATOOLS_DIR . '/assets/frontend/dist',
				self::DEV_SCRIPT,
				$this->get_config()
			);
